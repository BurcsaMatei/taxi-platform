...
// Component
// ==============================
export default function OpsMapLegacyEntry(): React.JSX.Element {
  ...
  const onGo = React.useCallback(
    (ev: React.FormEvent) => {
      ...
      setError("");
      void router.push(`/ops/${encodeURIComponent(raw)}/map`);
    },
    [cityId, router],
  );

  return (
    <main className={s.page} data-page="ops-map-legacy">
      <section className={s.card} aria-label="Alege orașul">
        <h1 className={s.title}>CONTROL CENTER / MAP</h1>
        <p className={s.subtitle}>
          Ruta nouă este <span className={s.mono}>/ops/[cityId]/map</span>. Alege orașul și
          continuă.
        </p>
