...
export default function BlueprintHud(props: BlueprintHudProps): React.JSX.Element {
  const { isOpen, onToggleOpen, districts, onResetView, onTeleport, onOpenDistrictPanel } = props;
  ...
  const blogItems = React.useMemo(() => {
    ...
    return posts.map((p) => ({
      ...
      // nu avem “external” pentru post acum
      externalHref: "",
    }));
  }, []);
            ...
            {districts.map((d) => {
              ...
              return (
                <div key={d.id} className={s.accWrap}>
                  ...
                  {/* Secondary action (existing): Open district panel */}
                  <button
                    type="button"
